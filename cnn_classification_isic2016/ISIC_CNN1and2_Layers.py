...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 15 22:53:05 2018
...
#Function to print testing accuracy
...
#2D Pooling Layer max pooling function (use 'MaxPooling2D' in Keras)
...
cnn_model1.compile(loss=LOSS,optimizer=OPTIMIZER1,metrics=METRICS)

print(cnn_model1.summary())

#filepath = 'Downloads/cifar/cifar_best_cnn_model1.hdf5'
filepath = 'isic2016/best_1cnn_model.hdf5'
...
cnn_model2.add(Conv2D(40,kernel_size=(5,5),padding="valid",kernel_initializer="glorot_uniform",input_shape=INPUT_SHAPE))
...
cnn_model2.compile(loss=LOSS,optimizer=OPTIMIZER2,metrics=METRICS)

print(cnn_model2.summary())

filepath = 'isic2016/best_2cnn_model.hdf5'
checkpoint = ModelCheckpoint(filepath, monitor='val_loss', verbose=1, save_best_only=True)
early_stopping_monitor = EarlyStopping(monitor='val_loss',patience=8)
...
print("Testing Accuracy for 1 CNN Layer Model w/ lr = 0.005")
print(GetMetrics(cnn_model1,x_test,y_test))
print(" ")
print("Testing Accuracy for 2 CNN Layer Model w/ lr = 0.005")
print(GetMetrics(cnn_model2,x_test,y_test))

# Get Optimization History Plot for both models
plotHistory(Tuning_cnn_model1, '1cnn_lr005')
plotHistory(Tuning_cnn_model2, '2cnn_lr005')
